      ...
      .col-md-3.col-lg-3.col-xl-3(v-if="authenticated")
        div
          q-card.relative-position.animate-scale
            ...
            div(style="margin:auto;")
              p.text-center {{fakeData.power}}
                ...
                small.block.light-paragraph.small Devices: {{parseInt(fakeData.devices)}}
                  q-icon(name="flash_on" color="yellow")
                small.block.light-paragraph Social: {{parseInt(fakeData.social)}}
                  q-icon(name="flash_on" color="yellow")
          q-card.animate-scale.relative-position
            ...
            p Users Invited: {{parseInt(fakeData.social/5 + 1)}}
            p Power from Invited Users: {{parseInt(fakeData.social)}}
      ...
      .col-md-9(style="min-width:300px;" )
        q-card.animate-scale.relative-position(v-if="authenticated")
          ...
          q-list( v-for="(device,index) in devices" :key="device.id")
            q-item.relative-position(v-if="!adBlock" style="height:80px;")
              //- | {{device.status}}
              q-item-side
...
export default {
  ...
  methods: {
    ...
    toggleDevice: async function(device) {
      this.deviceFakeData(device)
      var statusData = {
        deviceId: device.id
      }
      if (!device.toggle) {
        statusData.status = 'ACTIVE'
        this.fakeData.chartData[device.id] = this.fillData(true)
      } else {
        statusData.status = 'ONLINE'
        this.fakeData.chartData[device.id] = this.fillData(false)
      }
      var result = await this.api.device.updateStatus(statusData)
      setTimeout(() => {
    ...
    deviceFakeData(device) {
      if (!device.toggle) {
        this.fakeData.interval = setInterval(() => {
          this.fakeData.power += 1
          this.fakeData.social = this.fakeData.power * 0.3
          this.fakeData.devices = this.fakeData.power * 0.7
        }, 100)
      } else {
        // this.fakeData.power = 0
        clearInterval(this.fakeData.interval)
      }
    },
    init() {
      ...
      if (this.devices) {
        this.devices.forEach(el => {
          if (!this.fakeData.chartData[el.id]) {
            this.fakeData.chartData[el.id] = this.fillData()
          }
        })
      }
    }
